...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    DTypeId BusType;
	ssRegisterTypeFromNamedObject(S, BUS_NAME_RC_CHANNELS_SCALED, &BusType);
	...
	ssSetOutputPortWidth(S, 0, ENCODED_LEN_RC_CHANNELS_SCALED);
...
{
	int_T *busInfo = (int_T *) malloc(2*NFIELDS_BUS_RC_CHANNELS_SCALED*sizeof(int_T));
    if(busInfo == NULL) {
      ssSetErrorStatus(S, "Memory allocation failure");
      return;
    }
	encode_businfo_rc_channels_scaled(S, busInfo, 0);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	encode_vector_rc_channels_scaled(uvec, busInfo, yvec, 0);
}
