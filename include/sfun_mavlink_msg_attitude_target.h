...
#include "D:\000_simulink_mavlink-udp-git\simulink_mavlink\include\mavlink\v1.0\common\mavlink_msg_attitude_target.h"
#define BUS_NAME_ATTITUDE_TARGET "mavlink_attitude_target_t"
#define NFIELDS_BUS_ATTITUDE_TARGET 7
#define ENCODED_LEN_ATTITUDE_TARGET (MAVLINK_NUM_NON_PAYLOAD_BYTES + MAVLINK_MSG_ID_ATTITUDE_TARGET_LEN)
...
*/
static inline void encode_businfo_attitude_target(SimStruct *S, int_T *busInfo, const int_T offset)
{
	slDataTypeAccess *dta = ssGetDataTypeAccess(S);
	const char *bpath = ssGetPath(S);
	DTypeId BUSId = ssGetDataTypeId(S, BUS_NAME_ATTITUDE_TARGET);
...
a bitstream.
*/
static inline uint16_t encode_vector_attitude_target(const char *uvec, const int_T *busInfo, uint8_T *yvec, const int_T offset)
{
	mavlink_attitude_target_t ubus;
	...
	(void) memcpy(&ubus.type_mask, uvec + busInfo[offset+12], busInfo[offset+13]);
	
	mavlink_msg_attitude_target_encode(SYS_ID, COMP_ID, &msg_encoded, &ubus);
...
*/
static inline void decode_msg_attitude_target(const mavlink_message_t* msg_encoded, const int_T *busInfo, char *yvec, const int_T offset)
{
	mavlink_attitude_target_t ybus;
	mavlink_msg_attitude_target_decode(msg_encoded, &ybus);
	...
	(void) memcpy(yvec + busInfo[offset+12], &ybus.type_mask, busInfo[offset+13]);
}
